...
class Database {
  ...
  static Stream<List<LocalUser>> streamUsers() {
    return _db
        .collection('users')
        .snapshots()
        .map((QuerySnapshot list) => list.docs
            .map((DocumentSnapshot snap) => LocalUser.fromMap(snap.data()))
            .toList())
  ...
  static Stream<List<LocalUser>> getUsersByList(List<String> userIds) {
    final List<Stream<LocalUser>> streams = List();
    ...
    return StreamZip<LocalUser>(streams).asBroadcastStream();
  }
